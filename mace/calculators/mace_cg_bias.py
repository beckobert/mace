...
class MACECalculator_CG_Bias(Calculator):
    """MACE ASE Calculator
    args:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        config = data.load_from_mda_universe(universe, model_residues=self.models[0].residues)[0]
    ...
    def get_initial_structure(self, fn):
        ...
        if self.universe.dimensions is not None:
            pbc = [True] * 3
            cell = np.diag(self.universe.dimensions[:3])
        else:
            pbc = [False] * 3
            cell = None
        atoms = ase.Atoms(symbols=symbols, positions=positions, cell=cell, pbc=pbc)
        ase.io.write(fn, atoms, format='extxyz')
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        if self.model_type in ["MACE", "EnergyDipoleMACE"]:
            self.results["energy"] = (
                torch.mean(
                    torch.sum(ret_tensors['bias_node_energies'], dim=-1),
                    dim=0,
                ).cpu().item()
            ...
            self.results["node_energy"] = (
                torch.mean(
                    ret_tensors['bias_node_energies'],
                    dim=0,
                ).cpu().numpy()
            )
            ...
            if self.num_models > 1:
                self.results["energies"] = (
                    (torch.sum(ret_tensors['bias_node_energies'], dim=-1)).cpu().numpy()
                    * self.energy_units_to_eV
                )
